...
struct Box {
    HOSTDEV static bool intersect(const BoxData& data, const Float3 &r_orig, const Float3 &r_dir, float &t) {
    ...
    HOSTDEV static bool intersect(const BoxData& data, const BoxData &other) {
        if (data.box.xmin.x > other.box.xmax.x || data.box.xmin.y > other.box.xmax.y ||
            data.box.xmin.z > other.box.xmax.z)
            return false;
        if (data.box.xmax.x < other.box.xmin.x || data.box.xmax.y < other.box.xmin.y ||
            data.box.xmax.z < other.box.xmin.z)
            return false;
        return true;
    }

    HOSTDEV static Float3 normal(const BoxData& data, const Float3 &r_dir, const Float3 &intersection) {
